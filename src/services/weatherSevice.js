...
const API_KEY = "0e85d3ef1acdd29c790ea6fcd817ee3d";  // Use environment variable for API key
const BASE_URL = "https://api.openweathermap.org/data/2.5/";
...
const getWeatherData = async (infoType, searchParams) => {
    ...
    try {
        ...
        if (!res.ok) {
            throw new Error(`Failed to fetch ${infoType}: ${res.status} ${res.statusText}`);
        }
        return await res.json();
    } catch (error) {
        console.error("Error fetching weather data:", error);
        throw error;  // Rethrow so that calling functions can handle the error
    }
};
